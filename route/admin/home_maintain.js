...
exports.showhome=function(req,res){
	...
	var lun_data=[]
	db.query(`SELECT * FROM homes_maintenance limit `+i+`,10`,(err,data)=>{
		if(err){
		console.log(err);
			...
			for(var i=0;i<data.length;i++){
				data[i].hmaint_content=delHtmlTag(data[i].hmaint_content);
			}
...
exports.deleteImage=function(req,res){
	var id=req.body.id;
	//console.log(id);
	db.query(`DELETE FROM homes_maintenance WHERE ID=${id}`,(err,data)=>{
		if(err){
			console.error(err);
			res.status(500).send('database error').end();
		}else{
...
exports.addhome=function(req,res){
	res.render('admin/by_add.ejs',{});
}

//用来应付window里面重新重置的页面
exports.alterPage=function(req,res){
	res.render('admin/by_alter.ejs',{});
}

//添加除图片以外的数据
exports.addIm=function(req,res){
	var name=req.body.name;//家具保养标题
	var miao=req.body.miao;//家具保养内容
	var ishear=req.body.ishear;//是否是主要时间
	var time=new Date();//时间
	var date1=time.toLocaleTimeString();
	var date2=time.toLocaleDateString();
		date1=date1.split(':')
	time=date2+"-"+date1[0]+"-"+date1[1]+"-"+date1[2];
	//console.log(name,ishear);
	//将除图片以外的内容传入数据库
	db.query(`INSERT INTO homes_maintenance(hmaint_title,hmaint_content,news_time,isheadline) VALUE('${name}','${miao}','${time}','${ishear}')`,(err,data)=>{
		if(err){
			console.log(err);
			res.status(500).send("database error").end();
		}else{
	...
	if(newFileName){
		fs.rename(oldPath,newPath,(err)=>{
			if(err){
				...
				//将除图片以外的内容传入数据库
				db.query(`UPDATE homes_maintenance SET news_image='${newFileName}' where ID='${c_id}'`,(err,data)=>{
					if(err){
						console.log(err);
						res.status(500).send("database error").end();
					}else{
...
exports.alters=function(req,res){
	var id=req.body.id;
	//console.log(id);
	db.query(`SELECT * FROM homes_maintenance where homes_maintenance.ID='${id}'`,(err,data)=>{
		if(err){
			console.log(err);
			res.status(500).send('database error').end();
		}else if(data[0]==null){
...
exports.alterIm=function(req,res){
	...
	//将除图片以外的内容传入数据库
	db.query(`UPDATE homes_maintenance SET hmaint_title='${name}',hmaint_content='${miao}',news_time='${time}',isheadline='${ishear}' where ID='${id}'`,(err,data)=>{
		if(err){
			cosele.error(err);
			res.status(500).send('database error').end();
		}else{
