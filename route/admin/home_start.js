...
exports.showhome=function(req,res){
	...
	var promise=new Promise(function(resolve,reject){
		db.query(`SELECT ID,name,design_concept FROM product_piture_table where product_piture_table.iscarousel=0 and product_piture_table.isstar_pro=1 limit `+i+`,10`,(err,data)=>{
			if(err){
			console.log(err);
				...
				for(var i=0;i<data.length;i++){
					...
					lun_data[i].concept=data[i].design_concept;
				}
			}
			resolve(lun_data);
		})
...
exports.addhome=function(req,res){
	res.render('admin/sm_add.ejs',{});
}

//用来应付window里面重新重置的页面
exports.alterPage=function(req,res){
	res.render('admin/sm_alter.ejs',{});
}
//添加除图片以外的数据
exports.addIm=function(req,res){
	...
	//将除图片以外的内容传入数据库
	db.query(`INSERT INTO product_piture_table(name,iscarousel,design_concept,isstar_pro) VALUE('${name}','0','${miao}','1')`,(err,data)=>{
		if(err){
			console.log(err);
			res.status(500).send("database error").end();
		}else{
...
exports.addImage=function(req,res){
	...
	var data_img=req.files;
	var flag=0;
	for(let i=0;i<data_img.length;i++){
		if(data_img[i]){
			var ext=pathLib.parse(data_img[i].originalname).ext;//接收图片的后缀
			var oldPath=data_img[i].path;//获取 图片位置信息
			var newPath=data_img[i].path+ext;//获取图片位置信息+后缀
			var newFileName=data_img[i].filename+ext;//获取图片名字+后缀
		}else{
...
exports.alters=function(req,res){
	...
	var alter_data=[];
	var promise=new Promise(function(resolve,reject){
		db.query(`SELECT product_piture_table.ID,product_piture_table.name,product_piture_table.design_concept FROM product_piture_table where product_piture_table.ID='${id}'`,(err,data)=>{
			if(err){
			console.log(err);
				res.status(500).send('database error').end();
			}else if(data[0]==null){
				...
				for(var i=0;i<data.length;i++){
					...
					alter_data[i].carousel=data[i].design_concept;
				}
			}
			resolve(alter_data);
		})
...
exports.alterIm=function(req,res){
	...
	var miao=req.body.miao;
	//将除图片以外的内容传入数据库
	db.query(`UPDATE product_piture_table SET name='${name}',design_concept='${miao}' where ID='${id}'`,(err,data)=>{
		if(err){
			cosele.error(err);
			res.status(500).send('database error').end();
		}else{
